---
title: "Esfuerzo Pesquero Datos VMS México"
...
Extract fishing hours (Hrs/Km2) for all of the VMS Mexico data per year by 100th Lat and Lon bins
...
Create individual DB for all years and then create maps using those tables
...
```{r, eval=FALSE, echo=FALSE, dpi=300}
#Map
Mx_2019_Fedecoop <- ggplot() + 
  geom_sf(data = land_sf,
            fill = fishwatchr::gfw_palettes$map_country_dark[1],
            color = fishwatchr::gfw_palettes$map_country_dark[2],
          size=.1) +
  ...
  geom_tile(data = Tmp, aes(x = lon_bin, y = lat_bin, fill = Log_fishing_hours_sq_km), alpha = 0.5)+
  labs(fill = "Horas", title = "Horas de Pesca por Km2 en VMS         2019")+
  geom_sf(data=Zonas_Fedecoop,fill=NA, color="#58E8C6")+
  coord_sf(xlim = c(-117.5, -107.5), ylim = c(21, 29))
  ...
  #                     xmax = -84)
Mx_2019_Fedecoop
# ggsave("Mx_2019_Fedecoop.png", dpi=300)
